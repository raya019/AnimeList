...
import { onUnmounted, ref, onMounted, watch, onBeforeMount } from 'vue'
...
const fetchAnime = async () => {
  ...
  try {
    const genre = await getAnime(`anime?genres=${route.params.id}&page=${button.currentPage}`)

    data.value = genre.data.data
    button.lastPage = genre.data.pagination.last_visible_page
    count.value = genre.data.pagination.items.count
    total.value = genre.data.pagination.items.total
    perPage.value = genre.data.pagination.items.per_page
